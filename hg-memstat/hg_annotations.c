...
ANN_FUNC(void, AnnotatePrintMemoryUsage, char *file, int line, Word flags)
{
  const char *name = "AnnotatePrintMemoryUsage";
  ANN_TRACE("--#%d %s[%d] %s:%d\n", tid, name, (int)flags, file, line);
  DO_CREQ_v_W(VG_USERREQ__HG_PRINT_MEMORY_USAGE,   Word, flags);
}

ANN_FUNC(void, AnnotatePrintStats, char *file, int line, void *mu)
{
  const char *name = "AnnotatePrintStats";
  ANN_TRACE("--#%d %s[%p] %s:%d\n", tid, name, mu, file, line);
  DO_CREQ_v_W(VG_USERREQ__HG_PRINT_STATS,   void*, mu);
}

ANN_FUNC(void, AnnotateResetStats, char *file, int line, void *mu)
{
  const char *name = "AnnotateResetStats";
  ANN_TRACE("--#%d %s[%p] %s:%d\n", tid, name, mu, file, line);
  DO_CREQ_v_W(VG_USERREQ__HG_RESET_STATS,   void*, mu);
}
