#include "gcctsan_rt.h"
#include "gcctsan_atomic.h"
#include <stdint.h>
#include <stdlib.h>
#include <stdio.h>
#include <assert.h>
...
void dump_clock(char const* desc, timestamp_t const* clock) {
  ...
  fprintf(stderr, "gcctsan(%u): %s:", thr.id, desc);
  int i;
...
void            gcctsan_store   (addr_t addr) {
  int sz = 0;
  assert(addr != 0);
  assert(sz < 4);
  atomic_uint64_t* shadow = get_shadow(addr);
...
void gcctsan_enter(void const volatile* p)
{
  printf("enter %p\n", p);
}
...
typedef struct gcctsan_pthread_ctx_t {
  void*         (*start_routine)(void*);
  void*         arg;
  void*         ret;
} gcctsan_pthread_ctx_t;

static void* thread_wrapper(void* p) {
  gcctsan_pthread_ctx_t* ctx = (gcctsan_pthread_ctx_t*)p;
  handle_thread_start();
...
int     gcctsan_pthread_create    (pthread_t* th,
                                   pthread_attr_t* attr,
                                   void *(*start_routine)(void*),
                                   void *arg) {
  DBG("intercepting pthread_create");
  gcctsan_pthread_ctx_t* ctx = malloc(sizeof(gcctsan_pthread_ctx_t));
  if (ctx == 0)
    return ENOMEM;
...
int     gcctsan_pthread_join      (pthread_t th, void** retval) {
  DBG("intercepting pthread_join");
  gcctsan_pthread_ctx_t* ctx;
...
int     gcctsan_pthread_mutex_init    (pthread_mutex_t* mtx,
...
int     gcctsan_pthread_mutex_destroy (pthread_mutex_t* mtx) {
...
int     gcctsan_pthread_mutex_lock    (pthread_mutex_t* mtx) {
...
int     gcctsan_pthread_mutex_unlock  (pthread_mutex_t* mtx) {
...
int     gcctsan_pthread_cond_init     (pthread_cond_t* cv,
...
int     gcctsan_pthread_cond_destroy  (pthread_cond_t* cv) {
  int res = pthread_cond_destroy(cv);
  return res;
}


int     gcctsan_pthread_cond_signal   (pthread_cond_t* cv) {
  int res = pthread_cond_signal(cv);
  return res;
}


int     gcctsan_pthread_cond_broadcast(pthread_cond_t* cv) {
  int res = pthread_cond_broadcast(cv);
  return res;
}


int     gcctsan_pthread_cond_wait     (pthread_cond_t* cv,
                                       pthread_mutex_t* mtx) {
  handle_release(mtx, 1);
...
int     gcctsan_pthread_cond_timedwait(pthread_cond_t* cv,
                                       pthread_mutex_t* mtx,
