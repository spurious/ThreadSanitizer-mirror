...
static void             instrument_mop      (mop_ctx_t* ctx,
  ...
  if (ctx->ctx->func_ignore & (relite_ignore_mop | relite_ignore_rec)) {
    reason = "ignore file";
  ...
  if (is_sblock == 1 && ctx->ctx->func_ignore == relite_ignore_hist) {
    dbg(ctx->ctx, "resetting sblock due to ignore_hist");
    is_sblock = 0;
  }
