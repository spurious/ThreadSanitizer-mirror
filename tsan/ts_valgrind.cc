...
static void rtn_call(uintptr_t sp_after_call, 
                     uintptr_t pc_post_call_insn,
                     IGNORE_BELOW_RTN ignore_below) {
  ThreadId vg_tid = GetVgTid();
  ValgrindThread *thr = &g_valgrind_threads[vg_tid];
  uintptr_t sp = sp_after_call + 1;
